...
def _cli_parse(args):  # pragma: no coverage
    ...
    opt("-c", "--conf", action="append", metavar="FILE",
        help="load appconfig values from FILE.")
    opt("-C", "--param", action="append", metavar="NAME=VALUE",
        help="override appconfig values.")
...
class Route(object):
    ...
    def get_config(self, key, default=None):
        """ Lookup a appconfig field and return its value, first checking the
            route.appconfig, then route.app.appconfig."""
        depr(0, 13, "Route.get_config() is deprectated.",
                    "The Route.appconfig property already includes values from the"
                    " application appconfig for missing keys. Access it directly.")
...
class Bottle(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.config._add_change_listener(
            functools.partial(self.trigger_hook, 'appconfig'))
        ...
        if kwargs.get('catchall') is False:
            depr(0,13, "Bottle(catchall) keyword argument.",
                        ...
                        "configuration. Fix: `app.appconfig['catchall'] = False`")
        ...
        if kwargs.get('autojson') is False:
            depr(0, 13, "Bottle(autojson) keyword argument.",
                 ...
                 "configuration. Fix: `app.appconfig['json.enable'] = False`")
    ...
    catchall = DictProperty('appconfig', 'catchall')

    __hook_names = 'before_request', 'after_request', 'app_reset', 'appconfig'
...
class TemplatePlugin(object):
    """ This plugin applies the :func:`view` decorator to all routes with a
        `template` appconfig parameter. If the parameter is a tuple, the second
...
class ConfigDict(dict):
    ...
    def load_module(self, path, squash=True):
        """Load values from a Python module.
           ...
           >>> c = ConfigDict()
           >>> c.load_module('appconfig')
           {DEBUG: True, 'SQLITE.DB': 'memory'}
           >>> c.load_module("appconfig", False)
           {'DEBUG': True, 'SQLITE': {'DB': 'memory'}}
    ...
    def load_config(self, filename, **options):
        """ Load values from an ``*.ini`` style appconfig file.
            ...
            :param filename: The path of a appconfig file, or a list of paths.
            :param options: All keyword parameters are passed to the underlying
                :class:`python:appconfig.ConfigParser` constructor call.

        """
        options.setdefault('allow_no_value', True)
        if py3k:
            options.setdefault('interpolation',
                               appconfig.ExtendedInterpolation())
        conf = appconfig.ConfigParser(**options)
        conf.read(filename)
    ...
    def _define(self, key, default=_UNSET, help=_UNSET, validate=_UNSET):
        """ (Unstable) Shortcut for plugins to define own appconfig parameters. """
...
def _main(argv):  # pragma: no coverage
    ...
    for cfile in args.conf or []:
        ...
        except appconfig.Error as parse_error:
            _cli_error(parse_error)
        except IOError:
            _cli_error("Unable to read appconfig file %r" % cfile)
        except (UnicodeError, TypeError, ValueError) as error:
            _cli_error("Unable to parse appconfig file %r: %s" % (cfile, error))
